...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for animator in orm.Animator.objects.all():
            screening_csp = orm.Screening.objects.filter(animator = animator)
            farmer_video_tups = screening_csp.values_list('farmers_attendance__id','videoes_screened')
            farmers_tups = set([i[0] for i in farmer_video_tups])
            seen_videos = set([i[1] for i in farmer_video_tups])
            adoption_farmers = orm.PersonAdoptPractice.objects.filter(video__id__in = seen_videos).values_list('person__id', flat=True)
            animator.total_adoptions = len(filter(lambda x: x in farmers_tups, adoption_farmers))
            animator.save()
    
    def backwards(self, orm):
        "Write your backwards methods here."
        for animator in orm.Animator.objects.all():
            animator.total_adoptions = 0
            animator.save()
    ...
    models = {
        ...
        'dashboard.animator': {
            ...
            'total_adoptions': ('django.db.models.fields.PositiveIntegerField', [], {'default': '0'}),
