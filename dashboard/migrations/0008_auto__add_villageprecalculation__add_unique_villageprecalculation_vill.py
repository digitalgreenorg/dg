...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # RAHUL: Commenting to this avoid this constraint. It doesn't exist in the database
        #db.delete_unique(u'PERSON_ADOPT_PRACTICE', ['person_id', 'practice_id', 'DATE_OF_ADOPTION'])

        if not db.dry_run:
            paps = orm.PersonAdoptPractice.objects.values_list('id', 'person', 'video', 'date_of_adoption').order_by('date_of_adoption', 'person', 'video')
            for i in range(1, len(paps)):
                if paps[i][1] == paps[i-1][1] and paps[i][2] == paps[i-1][2] and paps[i][3] == paps[i-1][3]:
                    orm.PersonAdoptPractice.objects.get(id=paps[i][0]).delete()
