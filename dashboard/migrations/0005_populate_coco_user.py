...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        user_villages = {}
        user_partner = {}
        for user_per in orm['dashboard.UserPermission'].objects.all():
            if not user_district_dict.has_key(user_per.username):
                user_district_dict[user_per.username] = []
                user_villages[user_per.username] = []
            if user_per.district_operated:
                user_district_dict[user_per.username].append(user_per.district_operated.id)
                villages = orm['dashboard.Village'].objects.filter(block__district__id= user_per.district_operated.id).values_list('id', flat = True)
                user_villages[user_per.username] += villages
                user_partner[user_per.username] = user_per.district_operated.partner.id
        
        for user in orm['auth.User'].objects.all():
            if user_partner.has_key(user):
                obj = orm['dashboard.CocoUser'](user_id = user.id, partner_id = user_partner[user])
                obj.save()
                i = 0
                for vill in user_villages[user]:
                    obj.villages.add(vill)
                    i +=1
                print "user_id: " + str(user.id) +  " villages: " + str(i)  
