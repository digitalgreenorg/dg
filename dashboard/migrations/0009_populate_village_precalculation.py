...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        pmas = orm.PersonMeetingAttendance.objects.values_list('person','screening__date').order_by('person', 'screening__date')
        ...
        person_village_qs = orm.Person.objects.values_list('id','village')
        person_village = {}
        for id, village in person_village_qs:
            person_village[id] = village
        
        sc_min_date = orm.Screening.objects.aggregate(Min("date")).values()[0]
        today = datetime.date.today()
        paps = orm.PersonAdoptPractice.objects.values_list('person', 'date_of_adoption').order_by('person', 'date_of_adoption')
        ...
        for dt, vill_dict in main_data_dst.iteritems():
            for village_id, (adopt_active_att, active_att, tot_adopt_count) in vill_dict.iteritems():
                values_list.append("(%d, '%s', %d, %d, %d)" % (village_id, str(dt), adopt_active_att, active_att, tot_adopt_count))
                
        print "To insert", str(len(values_list)), "rows"
        for i in range(1, (len(values_list)/10000) + 2):
            db.execute("INSERT INTO "+orm.VillagePrecalculation._meta.db_table+"(village_id, date, total_adopted_attendees, total_active_attendees, total_adoption_by_active) VALUES "+','.join(values_list[(i-1)*10000:i*10000]))
