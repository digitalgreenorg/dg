...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        con = S3Connection(S3_ACCESS_KEY, S3_SECRET_KEY)
        bucket = con.get_bucket(BUCKET_NAME)
        update_count = id_not_found = 0
        rs = bucket.list("2/")
        # Creating thumbnails for all images in current directory
        for key in rs:
            key_name = key.name
            file_extension = os.path.splitext(key_name)
            farmer_id = file_extension[0][2:]
            try:
                farmer_id = int(farmer_id)
            except ValueError:
                print "Value error "+farmer_id    
                farmer_id = 0        
            person_qs = orm.Person.objects.filter(id = farmer_id)
            if person_qs:
                person_qs.update(image_exists = True)
                update_count = update_count + 1
            else:
                print 'id does not exist'+str(farmer_id)
                id_not_found = id_not_found + 1

        print "Total Updated = %d; Id not found = %d" % (update_count, id_not_found)
