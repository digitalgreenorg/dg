...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for obj in orm['videokheti.Crop'].objects.all():
            image_file = obj.image_file
            obj.image_file = ''.join([image_file.split('.')[0], '.jpg'])
            obj.save()
        
        for obj in orm['videokheti.TimeYear'].objects.all():
            image_file = obj.image_file
            obj.image_file = ''.join([image_file.split('.')[0], '.jpg'])
            obj.save()
        
        for obj in orm['videokheti.ActionType'].objects.all():
            image_file = obj.image_file
            obj.image_file = ''.join([image_file.split('.')[0], '.jpg'])
            obj.save()
        
        for obj in orm['videokheti.Method'].objects.all():
            image_file = obj.image_file
            obj.image_file = ''.join([image_file.split('.')[0], '.jpg'])
            obj.save()
