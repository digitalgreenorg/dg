...
def get_user_videos(user_id):
    ...
    user_states = State.objects.filter(
        district__block__village__in=villages).distinct().values_list('id', flat=True)
    if coco_user.type_of_cocouser not in [3, 4]:
        user_videos = coco_user.videos.filter(
            category__parent_category_id=coco_user.type_of_cocouser).values_list('id', flat=True)
        # FIRST GET VIDEOS PRODUCED IN STATE WITH SAME PARTNER
        videos = Video.objects.filter(village__block__district__state__in=user_states, partner_id=coco_user.partner_id,
                                      category__parent_category_id=coco_user.type_of_cocouser).values_list('id', flat=True)
        # Get videos screened to allow inter partner sharing of videos
        person_obj_list = set(Person.objects.filter(
            village__in=villages, partner_id=coco_user.partner_id).values_list('screening__videoes_screened', flat=True))
        videos_seen = Video.objects.filter(id__in=list(
            person_obj_list), category__parent_category_id=coco_user.type_of_cocouser).values_list('id', flat=True)
    else:
        ...
        videos = Video.objects.filter(village__block__district__state__in=user_states,
                                      partner_id=coco_user.partner_id).values_list('id', flat=True)

        # Get videos screened to allow inter partner sharing of videos
        videos_seen = set(Person.objects.filter(village__in=villages, partner_id=coco_user.partner_id).values_list(
            'screening__videoes_screened', flat=True))
...
def get_user_mediators(user_id):
    ...
    user_districts = District.objects.filter(
        block__village__in=villages).distinct().values_list('id', flat=True)
    mediators_from_same_district = Animator.objects.filter(
        district__in=user_districts, partner_id=partner).distinct().values_list('id', flat=True)
...
def assign_partner(bundle):
    ...
    if partner_id:
        bundle.data['partner'] = "/coco/api/v2/%s/%s/" % (
    ...
    def read_list(self, object_list, bundle):
        try:
            ...
            if meta_auth_container and len(meta_auth_container):
                apikey = bundle.request.META.get(
...
class MediatorResource(BaseResource):
    ...
    district = fields.ForeignKey(
        'coco.api.DistrictResource', 'district', null=True)
    ...
    def obj_update(self, bundle, **kwargs):
        ...
        existing_vil_ids = AnimatorAssignedVillage.objects.filter(
            animator__id=mediator_id).values_list('village__id', flat=True)
        ...
        if len(list(set(vil_id_list) & set(existing_vil_ids))) != len(vil_id_list):
            ...
            del_objs = AnimatorAssignedVillage.objects.filter(
                animator__id=mediator_id).delete()
    ...
    def hydrate_partner(self, bundle):
        partner_id = get_user_partner_id(bundle.request.user.id)
        if partner_id:
            bundle.data['partner'] = "/coco/api/v2/partner/" + \
                str(partner_id)+"/"
        return bundle
...
class VillageResource(ModelResource):
    ...
    class Meta:
        ...
        queryset = Village.objects.select_related(
            'block__district__state__country').all()
...
class VideoResource(BaseResource):
    ...
    production_team = fields.ToManyField(
        'coco.api.MediatorResource', 'production_team')
    direct_beneficiaries = fields.ToManyField(
        'coco.api.DirectBeneficiariesResource', 'direct_beneficiaries', null=True)
    ...
    category = fields.ForeignKey(
        'coco.api.CategoryResource', 'category', null=True)
    subcategory = fields.ForeignKey(
        'coco.api.SubCategoryResource', 'subcategory', null=True)
    videopractice = fields.ToManyField(
        'coco.api.VideoPracticeResource', 'videopractice', null=True)
    ...
    dehydrate_category = partial(foreign_key_to_id, field_name='category', sub_field_names=[
                                 'id', 'category_name', 'parent_category'])
    dehydrate_subcategory = partial(
        foreign_key_to_id, field_name='subcategory', sub_field_names=['id', 'subcategory_name'])
    ...
    hydrate_subcategory = partial(
        dict_to_foreign_uri, field_name='subcategory', resource_name='subcategory')

    hydrate_videopractice = partial(
        dict_to_foreign_uri_m2m, field_name='videopractice', resource_name='videopractice')
    hydrate_tags = partial(dict_to_foreign_uri_m2m,
                           field_name='tags', resource_name='tag')
    hydrate_production_team = partial(
        dict_to_foreign_uri_m2m, field_name='production_team', resource_name='mediator')
    hydrate_direct_beneficiaries = partial(
        dict_to_foreign_uri_m2m, field_name='direct_beneficiaries', resource_name='directbeneficiaries')
    hydrate_partner = partial(assign_partner)

    class Meta:
        max_limit = None
        queryset = Video.objects.prefetch_related(
            'village', 'language', 'production_team', 'partner', 'category', 'subcategory').all()
        resource_name = 'video'
        authentication = MultiAuthentication(
            SessionAuthentication(), AnonymousGETAuthentication())
        authorization = VideoAuthorization()
        validation = ModelFormValidation(form_class=VideoForm)
        always_return_data = True
        excludes = ['duration', 'related_practice', 'time_created',
                    'time_modified', 'review_status', 'video_grade']
    ...
    def hydrate_village(self, bundle):
        ...
        if village and not hasattr(bundle, 'village_flag'):
            try:
                ...
                bundle.data['village'] = "/coco/api/v2/village/" + \
                    str(village_id)+"/"
...
def map_dict():
    data_dict = {}
    dobj = DirectBeneficiaries.objects.values(
        'id', 'direct_beneficiaries_category')
    for item in dobj:
...
class ScreeningResource(BaseResource):
    ...
    frontlineworkerpresent = fields.ToManyField(
        'coco.api.FrontLineWorkerPresentResource', 'frontlineworkerpresent', related_name='screening')
    videoes_screened = fields.ToManyField(
        'coco.api.VideoResource', 'videoes_screened', related_name='screening')
    farmer_groups_targeted = fields.ToManyField(
    ...
    dehydrate_parentcategory = partial(
        foreign_key_to_id, field_name='parentcategory', sub_field_names=['id', 'parent_category_name'])
    dehydrate_animator = partial(
        foreign_key_to_id, field_name='animator', sub_field_names=['id', 'name'])
    hydrate_village = partial(dict_to_foreign_uri, field_name='village')
    hydrate_animator = partial(
        dict_to_foreign_uri, field_name='animator', resource_name='mediator')
    hydrate_farmer_groups_targeted = partial(
        dict_to_foreign_uri_m2m, field_name='farmer_groups_targeted', resource_name='group')
    hydrate_videoes_screened = partial(
        dict_to_foreign_uri_m2m, field_name='videoes_screened', resource_name='video')
    hydrate_frontlineworkerpresent = partial(
        dict_to_foreign_uri_m2m, field_name='frontlineworkerpresent', resource_name='frontlineworkerpresent')
    hydrate_partner = partial(assign_partner)
    ...
    def obj_create(self, bundle, **kwargs):
        ...
        if pma_list:
            ...
            raise PMANotSaved('Screening with details: ' + str(bundle.data) +
                              ' can not be saved because attendance list is not available')
    ...
    def all_category(self, bundle, list_queryset):
        ...
        for pma in filter(None, list_queryset):
            if isinstance(pma.get('category'), unicode):
                try:
                    int_pma_db_list = [
                        int(item) for item in ast.literal_eval(pma.get('category'))]
                except:
                    try:
                        int_pma_db_list = [
                            int(item.get('id')) for item in ast.literal_eval(pma.get('category'))]
                    except:
                        pass
    ...
    def dehydrate_category(self, bundle):
        queryset = bundle.obj.personmeetingattendance_set.values(
            'id', 'person_id', 'person__person_name', 'category')
...
class PersonResource(BaseResource):
    ...
    def dehydrate_label(self, bundle):
        # for sending out label incase of dropdowns
        v_field = getattr(bundle.obj, 'village').village_name
        f_field = getattr(bundle.obj, 'father_name') if (
            getattr(bundle.obj, 'father_name') != None) else ""
...
class PersonAdoptVideoResource(BaseResource):
    ...
    class Meta:
        max_limit = None
        queryset = PersonAdoptPractice.objects.prefetch_related('person__village', 'video', 'animator', 'person__group', 'person', 'partner').filter(
        ...
        always_return_data = True
        excludes = ['time_created', 'time_modified',
                    'verification_status', 'non_negotiable_check', 'verified_by']
    dehydrate_video = partial(
        foreign_key_to_id, field_name='video', sub_field_names=['id', 'title'])
    dehydrate_village = partial(
        foreign_key_to_id, field_name='village', sub_field_names=['id', 'village_name'])
...
class CategoryResource(ModelResource):
    parent_category = fields.ForeignKey(
        ParentCategoryResource, 'parent_category', null=True)
    ...
    hydrate_parent_category = partial(
        dict_to_foreign_uri, field_name='parent_category', resource_name='parentcategory')
...
class SubCategoryResource(ModelResource):
    ...
    class Meta:
        ...
        authentication = MultiAuthentication(
            SessionAuthentication(), AnonymousGETAuthentication())
        authorization = Authorization()
    dehydrate_category = partial(
        foreign_key_to_id, field_name='category', sub_field_names=['id', 'category_name'])
    hydrate_category = partial(
        dict_to_foreign_uri, field_name='category', resource_name='category')
...
class DirectBeneficiariesResource(BaseResource):
    ...
    hydrate_category = partial(
        dict_to_foreign_uri_m2m, field_name='category', resource_name='category')
