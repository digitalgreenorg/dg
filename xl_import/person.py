...
def add_person(file, user_id, block_id):
    ...
    for row in rows:
        if str(row['Village_Name']) not in village_map:
            ...
            try:
                ...
                error += 1
                wrtr.writerow([row['Village_Name']] )
    ...
    for row in rows2:
        ...
        try:
            ...
            wrtr.writerow([' '.join([str(row['Member_Name']),str(row['Member_Surname'])])])       #wrtr.writerow([row['Member_Name'],row['Member_Surname']] )
