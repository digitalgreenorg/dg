...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        top_practice_dict = make_name_id_dictionary(orm['dashboard.PracticeSector'])
        sub_practice_dict = make_name_id_dictionary(orm['dashboard.PracticeSubSector'])
        utility_dict = make_name_id_dictionary(orm['dashboard.PracticeMain'])
        type_dict= make_name_id_dictionary(orm['dashboard.PracticeSub'])
        subject_dict = make_name_id_dictionary(orm['dashboard.PracticeSubject'])
        print "Fetched all levels data into dictionary"
        
        #Parse Tab Delemited Excel txt
        file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), r"outputfile.txt")
        excel_records = []
        with open(file_path, 'r') as f:
            lines = f.readlines()  # no header to splice
            excel_records = map(lambda x: x.strip('\n').split(';'), lines)
            for record in excel_records:
                orm['video_practice_map.PracticeCombination'].objects.create(top_practice=top_practice_dict[record[0].lower()],
                                                                    sub_practice=sub_practice_dict[record[1].lower()],
                                                                    utility=utility_dict[record[2].lower()],
                                                                    type=type_dict[record[3].lower()],
                                                                    subject=subject_dict[record[4].lower()])
        print "Done filling up foreign key relations"

    def backwards(self, orm):
        orm['video_practice_map.PracticeCombination'].objects.all().delete()
