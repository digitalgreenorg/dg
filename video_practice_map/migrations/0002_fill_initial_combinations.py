...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        top_practice_dict = make_name_id_dictionary(orm['dashboard.TopPractice'])
        sub_practice_dict = make_name_id_dictionary(orm['dashboard.SubPractice'])
        type_dict = make_name_id_dictionary(orm['dashboard.PracticeType'])
        subject_dict = make_name_id_dictionary(orm['dashboard.PracticeSubject'])
        utility_dict = make_name_id_dictionary(orm['dashboard.PracticeUtility'])
        print "Fetched all levels data into dictionary"
        
        #Parse Tab Delemited Excel txt
        file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), r"data_files/level_combinations.txt")
        excel_records = []
        ...
        vid_qs = orm['dashboard.Video'].objects.filter(id__in = set([i[0] for i in excel_records]))
        vid_dict = {}
        for vid in vid_qs:
            vid_dict[vid.id] = vid 
        print "Fetched all video data in dictionary"
        
        new_practice_dict = {}
        for entry in excel_records:
            pr_key = ';'.join(entry[2:])
            if pr_key not in new_practice_dict:
                new_prac_obj = orm['video_practice_map.PracticeCombination']()
            ...
            if entry[0] in vid_dict:
                ...
                orm['video_practice_map.VideoPractice'].objects.create(video=vid, practice=new_prac_obj)
    ...
    complete_apps = ['dashboard', 'video_practice_map']
