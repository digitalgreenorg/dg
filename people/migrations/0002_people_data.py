...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Animator.objects.bulk_create([Animator(user_created_id=x.user_created_id, time_created=x.time_created, user_modified_id=x.user_modified_id, time_modified=x.time_modified,
                                               old_coco_id=x.id, name=x.name, age=x.age, gender=x.gender, csp_flag=x.csp_flag, camera_operator_flag=x.camera_operator_flag, facilitator_flag=x.facilitator_flag, phone_no=x.phone_no, address=x.address, partner=orm['programmes.Partner'].objects.get(old_coco_id=x.partner_id), village=None if x.village is None else Village.objects.get(old_coco_id=x.village_id), district=None if x.district_id is None else orm['geographies.District'].objects.get(old_coco_id=x.district_id), total_adoptions=x.total_adoptions) for x in orm['dashboard.Animator'].objects.all()])

        print "Animators Added"

        db.create_index('people_animator', ['old_coco_id'])
        print "index created for animators"

        Assigned = Animator.assigned_villages.through
        Assigned.objects.bulk_create([Assigned(user_created_id=x.user_created_id, time_created=x.time_created, user_modified_id=x.user_modified_id, time_modified=x.time_modified,
                                               animator_id=Animator.objects.get(old_coco_id=x.animator_id).id, village_id=Village.objects.get(old_coco_id=x.village_id).id, start_date=x.start_date) for x in orm['dashboard.Animator'].assigned_villages.through.objects.all()])

        print "Animators Assigned Villages Added"

        chunk = 1000

        PersonGroup = orm['people.PersonGroup']
        Partner = orm['programmes.Partner']
        dashboard_persongroup = orm['dashboard.PersonGroups'].objects.all()
        for i in range(0, dashboard_persongroup.count()/chunk + 1):
            dashboard_persongroup_slice = dashboard_persongroup[i*chunk:(i+1)*chunk]
            print "Person Group Added %d" % (i*chunk)
            PersonGroup.objects.bulk_create([PersonGroup(user_created_id=x.user_created_id, time_created=x.time_created, user_modified_id=x.user_modified_id, time_modified=x.time_modified,
                                                         old_coco_id=x.id, group_name=x.group_name, days=x.days, timings=x.timings, time_updated=x.time_updated, partner=Partner.objects.get(old_coco_id=x.partner_id), village= Village.objects.get(old_coco_id=x.village_id))
                                             for x in dashboard_persongroup_slice])

        print "person group created"

        db.create_index('people_persongroup', ['old_coco_id'])
        print "index created for person group"
        ...
        for i in range(0, dashboard_persons.count()/chunk + 1):
            ...
            Person.objects.bulk_create([Person(user_created_id=x.user_created_id, time_created=x.time_created, user_modified_id=x.user_modified_id, time_modified=x.time_modified,
                                               old_coco_id=x.id, person_name=x.person_name, father_name=x.father_name, 
        ...
        db.create_index('people_person', ['old_coco_id'])
        print "index created for person"
