...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        Cocouser = orm['coco.CocoUser']
        Partner = orm['programs.Partner']
        Village = orm['geographies.Village']
        Cocouser.objects.bulk_create([Cocouser(user_created_id=x.user_created_id, time_created=x.time_created, user_modified_id=x.user_modified_id, time_modified=x.time_modified,
                                               old_coco_id=x.id, user_id=x.user_id, partner=Partner.objects.get(old_coco_id=x.partner_id))
                                      for x in orm['dashboard.CocoUser'].objects.all()])

        print "CocoUsers Added"

        db.create_index('coco_cocouser', ['old_coco_id'])
        print "index created for animators"

        Assigned = Cocouser.villages.through
        Assigned.objects.bulk_create([Assigned(cocouser_id=Cocouser.objects.get(old_coco_id=x.cocouser_id).id, village_id=Village.objects.get(old_coco_id=x.village_id).id) for x in orm['dashboard.CocoUser'].villages.through.objects.all()])

        print "Cocouser Assigned Villages Added"
