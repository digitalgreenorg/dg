...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        person_adopt_practice = orm['activities.PersonAdoptPractice']
        video = orm['videos.Video']
        person = orm['people.Person']
        person_group = orm['people.PersonGroup']
        animator = orm['people.Animator']
        partner = orm['programs.Partner']

        # West Singhbum District and Pragati Partner and assign it to Pradan Partner

        partner_obj = partner.objects.get(id=1)

        #Screening Data
        screening.objects.filter(village__block__district__id=4, partner_id=7).update(partner=partner_obj)

        #Adoption Data
        person_adopt_practice.objects.filter(person__village__block__district__id=4, partner_id=7).update(partner=partner_obj)

        #Video Data
        video.objects.filter(village__block__district__id=4, partner_id=7).update(partner=partner_obj)

        #Person Data
        person.objects.filter(village__block__district__id=4, partner_id=7).update(partner=partner_obj)

        #PersonGroup Data
        person_group.objects.filter(village__block__district__id=4, partner_id=7).update(partner=partner_obj)

        #Animator Data
        animator.objects.filter(village__block__district__id=4, partner_id=7).update(partner=partner_obj)

        # Muzaffarpur District and Green Foundation Partner and assign it to BRLPS Partner

        partner_obj = partner.objects.get(id=11)
        #Screening Data
        screening.objects.filter(village__block__district__id=29, partner_id=3).update(partner=partner_obj)

        #Adoption Data
        person_adopt_practice.objects.filter(person__village__block__district__id=29, partner_id=3).update(partner=partner_obj)

        #Video Data
        video.objects.filter(village__block__district__id=29, partner_id=3).update(partner=partner_obj)

        #Person Data
        person.objects.filter(village__block__district__id=29, partner_id=3).update(partner=partner_obj)

        #PersonGroup Data
        person_group.objects.filter(village__block__district__id=29, partner_id=3).update(partner=partner_obj)

        #Animator Data
        animator.objects.filter(village__block__district__id=29, partner_id=3).update(partner=partner_obj)

        # Damdaha Block and BRLPS Partner before '31-12-2012' and assign it to ASA Partner

        partner_obj = partner.objects.get(id=6)

        #Screening Data
        screening.objects.filter(village__block__id=98, partner_id=11, date__lte=datetime.date(2012,12,31)).update(partner=partner_obj)

        #Adoption Data
        person_adopt_practice.objects.filter(person__village__block__id=98, partner_id=11, date_of_adoption__lte=datetime.date(2012,12,31)).update(partner=partner_obj)

        #Video Data
        video.objects.filter(village__block__id=98, partner_id=11, video_production_end_date__lte=datetime.date(2012,12,31)).update(partner=partner_obj)
    ...
    complete_apps = ['videos', 'people', 'activities']
    symmetrical = True
