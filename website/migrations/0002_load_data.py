...
#        country migrations
        ...
        for j in country:
            temp=Country(countryCode=str(j.id),countryName=j.country_name)
            temp.save()
...
#        farmer migrations
        ...
        farmer=orm['dashboard.Person'].objects.all()[:5000]
        for i in farmer:
            if i.image_exists:
...
#        partner migrations
        ...
        for i in partner:
            ...
            temp.save()
            district=i.district_set.all()
            farmer=[]
            for j in district:
                farmer.extend(list(Farmer.objects.filter(district=j.district_name)))
            for j in farmer:
                temp.farmers.add(j)
...
#        video migrations
        ...
        video=orm['dashboard.Video'].objects.all()[:500]
        for i in video:
...
#            collections migrations
        ...
        for i in collection_comb:
            ...
            lang=i[1]
            topic=i[2]
            country=Country.objects.get(countryCode=str(orm['dashboard.Partners'].objects.get(id=partner).district_set.all()[0].state.country.id))
            videos = orm['dashboard.Video'].objects.filter(village__block__district__partner_id=partner,language=lang,related_practice__practice_topic_id = topic)
            if len(set(videos.values_list('related_practice__practice_sector_id')))==1 and videos[0].related_practice.practice_sector!= None:
                sector = videos[0].related_practice.practice_sector.name
            else:
                sector = ""
            ...
            for vid in videos:
                try:
                    temp.videos.add(Video.objects.get(uid=str(vid.id)))
                except:
                    pass
            counter = counter+1
            
        collection_comb = orm['dashboard.Video'].objects.exclude(related_practice__practice_subject_id=None).values_list('village__block__district__partner_id','language_id','related_practice__practice_subject_id').annotate(c= Count('id')).filter(c__gte=5,c__lte=25)
        for i in collection_comb:
            partner=i[0]
            lang=i[1]
            sub=i[2]
            country=Country.objects.get(countryCode=str(orm['dashboard.Partners'].objects.get(id=partner).district_set.all()[0].state.country.id))
            videos = orm['dashboard.Video'].objects.filter(village__block__district__partner_id=partner,language=lang,related_practice__practice_subject_id = sub)
            if len(set(videos.values_list('related_practice__practice_sector_id')))==1 and videos[0].related_practice.practice_sector!= None:
                sector = videos[0].related_practice.practice_sector.name
            else:
                sector = ""
...
#        activity migrations
        ...
        for i in farmer:
            ...
            videos_watched=orm['dashboard.PersonMeetingAttendance'].objects.filter(person_id = person.id).values_list('screening__videoes_screened__title', 'screening__date').order_by('screening__date')
            videos_adopted=orm['dashboard.PersonAdoptPractice'].objects.filter(person_id = person.id).values_list('video__title', 'date_of_adoption').order_by('date_of_adoption')
            person_activity=[]
            vw=va=0
            for j in range(len(videos_watched)+len(videos_adopted)):
                if(vw==len(videos_watched)):
                    temp=[i.name,videos_adopted[va][0],videos_adopted[va][1],'A']
                    person_activity.append(temp)
                    va=va+1
                elif (va==len(videos_adopted)):
                    temp=[i.name,videos_watched[vw][0],videos_watched[vw][1],'S']
                    person_activity.append(temp)
                    vw=vw+1
        ...
        for i in range(len(activity)):
            for j in range(len(activity[i])):
                if(activity[i][j][3]=='S'):
                    temp = Activity(uid=str(counter),date=activity[i][j][2],title="Farmer Activity",textContent="Today "+activity[i][j][0]+" saw video titled "+ activity[i][j][1],type='F')
                    temp.save()
                    counter=counter+1
                else:
                    temp=Activity(uid=str(counter),date=activity[i][j][2],title="Farmer Activity",textContent="Today "+activity[i][j][0]+" adopted video titled "+ activity[i][j][1],type='F')
                    temp.save()
                    counter=counter+1
        
        partner=orm['dashboard.Partners'].objects.all().order_by('date_of_association')
                ...
                for k in block:
                    ...
                    for l in village:
                        if partner_village_count==10 and repeat_flag==False:
                            temp = Activity(uid=str(counter),date=last_date,title="Partner Activity",textContent="Our Partner "+i.partner_name+" has expanded their work to reach 10 villages today",type='P')
                            temp.save()
                            counter = counter + 1
                            repeat_flag=True
                        if partner_village_count==50 and repeat_flag==False:
                            temp = Activity(uid=str(counter),date=last_date,title="Partner Activity",textContent="Our Partner organisation "+i.partner_name+" have expanded there work in 50 villages today",type='P')
                            temp.save()
                            counter = counter + 1
                            repeat_flag=True
                        if partner_village_count==100 and repeat_flag==False:
                            temp = Activity(uid=str(counter),date=last_date,title="Partner Activity",textContent="Our Partner organisation "+i.partner_name+" have expanded there work in 100 villages today",type='P')
                            temp.save()
                            counter = counter + 1
                            repeat_flag=True
                        ...
                        temp = Activity(uid=str(counter),date=last_date,title="Partner Activity",textContent="Our Partner organisation "+i.partner_name+" started working in Village "+l.village_name+" today",type='P')
                        temp.save()
                        counter = counter + 1
                        ...
                        for m in video:
                            temp = Activity(uid=str(counter),date=m.video_production_end_date,title="Partner Activity",textContent="A new video titled "+m.title+" was produced by our Partner "+i.partner_name,type='P')
                            temp.save()
                            counter = counter + 1                
        #interest migrations
                ...
                if j.expressed_question!='':
                    for k in j.screening.videoes_screened.all():
                        if len(Video.objects.filter(uid=str(k.id)))==1:
                            temp=Comment(uid=str(counter),date=j.screening.date,text=j.expressed_question,isOnline=False,farmerUID=i,videoUID =Video.objects.get(uid=str(k.id)) )
                            temp.save()
