...
def collection_view(request):
    ...
    for vid in videos:
        if i==int(videoID):
            ...
            if '' in tag_list:
                tag_list.remove('')
            video_dict={'videoID':videoID,'title':vid.title,'description':vid.description,'youtubeID':vid.youtubeID,'tags':vid.tags,'date':vid.date,'tags':tag_list}
        i=i+1
        time=time+vid.duration
