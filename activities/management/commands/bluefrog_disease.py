...
class Command(BaseCommand):
	def handle(self, *args, **options):
		...
		xml_file = open("ap/disease_tag.xml", 'w')
		xml_file.write(req.content)
		xml_file.close()
		# partner=Partner.objects.get(id=50)
		csv_file = open('ap/disease_tag.csv', 'wb')
		...
		for data_iterable in data:
			tag_code = data_iterable.get('Disease_id')
			tag_name = data_iterable.get('Disease_name')
			tag_regional_name = data_iterable.get('Disease_name_telugu')
