...
class Command(BaseCommand):
	def handle(self, *args, **options):
		...
		contents = url.read()
		xml_file = open("jslps_data_integration_files/jslps-hnn-person-new4.xml", 'w')
		xml_file.write(contents)
		xml_file.close()

		partner = Partner.objects.get(id = 24)
		user_obj = User.objects.get(username="jslps_bot")
		csv_file = open('jslps_data_integration_files/jslps-hnn-person-new4.csv', 'wb')
		wtr = csv.writer(csv_file, quoting=csv.QUOTE_ALL)
		tree = ET.parse('jslps_data_integration_files/jslps-hnn-person-new4.xml')
		root = tree.getroot()
		
		for c in root.findall('GroupMemberDataNew4'):
