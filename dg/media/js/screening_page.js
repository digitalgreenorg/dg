...
add_link_html =
  '<h2>Add New Row <img src = "/media/img/admin/icon_addlink.gif" width="10" height="10"></h2>';
...
jQuery(function ($) {
  ...
  if (vil_id > 0) {
    ...
    $(
      "div.inline-group div.tabular table input.vForeignKeyRawIdAdminField"
    ).hide();
    ...
    $(".form-row.farmer_groups_targeted div, .form-row.animator div").append(
      '<text class="error_msg" style="font-size:20px;float:center; margin-left:50px;margin-top:70px;">Select Village to Enable</text>'
    );
    hideStatus();
  }
});
...
function clear_table(tab) {
  ...
  if (!is_edit) {
    if (tab.find("tbody").length > 0) tab.find("tbody").html("");
    return;
  }
...
function initialize_add_screening() {
  ...
  table
    .find("td.delete input:checkbox:checked")
    .parent("td")
    .parent("tr")
    .addClass("deleted_row")
    .hide();

  // "Add"-button in bottom of inline for adding new rows
  tabu
    .find("fieldset")
    .after('<a class="add" href="#">' + add_link_html + "</a>");
