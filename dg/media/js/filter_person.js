add_link_html =
  '<h2>Add New Row <img width="10" height="10" src="/media/img/admin/icon_addlink.gif"/></h2>';
    ...
    $.getJSON("/feeds/persons/" + $(selected).val() + "/", function (j) {
      ...
      for (var i = 0; i < j.length; i++) {
        ...
        options =
          ...
          parseInt(j[i].pk) +
          '">' +
          j[i].fields["person_name"] +
          "</option>";
        $("#id_personmeetingattendance_set-" + counter + "-person").html(
          options
        );
        $(
          "#id_personmeetingattendance_set-" + counter + "-person option:first"
        ).attr("selected", "selected");
        $("#id_personmeetingattendance_set-" + counter + "-person").attr(
          "disabled",
          false
        );
        counter++;
