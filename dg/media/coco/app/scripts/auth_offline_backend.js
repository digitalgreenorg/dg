...
define([
  ...
  var login = function(username, password, language, partner_name, type_of_cocouser){
      var dfd = new $.Deferred();
      User.fetch({
          success: function(){
              if(username==User.get("username") && password==User.get("password"))
              {
                  save_login_state(username, password, language, partner_name, type_of_cocouser, true)
                      .done(function(){
  ...
  var save_login_state = function(username, password, language, partner_name, type_of_cocouser, loggedin){
      var dfd = new $.Deferred();
      User.save({'username':username, 'password':password, 'loggedin':loggedin, 'language':language, 'partner_name': partner_name, 'type_of_cocouser': type_of_cocouser},{
          success: function(){
