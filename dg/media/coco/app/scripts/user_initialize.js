...
define([
    ...
    var run = function () {
        ...
        $.validator.addMethod(
            "allowedChar",
            validateUniCodeChars,
            "Enter a string."
        );
        $.validator.addMethod(
            "validateDate",
            validateDate,
            "Enter the date in the form of YYYY-MM-DD."
        );
        $.validator.addMethod(
            "validateTime",
            validateTime,
            "Enter the time in the form of HH:MM. Use 24 hour format"
        );
        $.validator.addMethod(
            "dateOrder",
            dateOrder,
    ...
    function reset_database_check() {
        ...
        Auth.check_login().done(function () {
    ...
    function dateOrder(value, element, options) {
        var check = false;
        var start = $("#" + options.production_date).val();
