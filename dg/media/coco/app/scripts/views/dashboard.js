...
define([
    ...
    var DashboardView = Backbone.Layout.extend({
        ...
        afterRender: function () {
            ...
            for (var member in configs) {
                ...
                if (configs[member].dashboard_display) {
                    ...
                    enable_months =
                        configs[member].dashboard_display.enable_months;
                }
                ...
                if (listing || add) {
                    ...
                    if (add)
                        ...
                        $("#dashboard_items_add").append(
                            "<li class='disabled'><a><i class='glyphicon glyphicon-plus-sign' title='You are not allowed to add this currently'></a></li>"
                        );
                }
            }
            this.upload_entries = upload_collection.length;
            ...
            // });
            $("#main-navbar").on("click", function () {
                if ($(window).width() < 768) $(".collapse").collapse("hide");
            });
            if (User.isOnline()) {
                    ...
                    that.upload()
                        ...
                        .fail(function (error) {
                            ...
                            notifs_view.add_alert({
                                notif_type: "error",
                                message:
                                    "Sync Incomplete. Failed to finish upload : " +
                                    error,
                            });
                        })
                        .always(function () {
                                ...
                                that.inc_download({
                                    ...
                                    .done(function () {
                                        ...
                                        notifs_view.add_alert({
                                            notif_type: "success",
                                            message:
                                                "Incremental download successfully finished",
                                        });
                                    })
                                    .fail(function (error) {
                                        ...
                                        notifs_view.add_alert({
                                            notif_type: "error",
                                            message:
                                                "Sync Incomplete. Failed to do Incremental Download: " +
                                                error,
                                        });
                                    });
                            }
                ...
                .fail(function (model, error) {
                    ...
                    if (error == "Not Found") {
                        that.render().done(function () {
                            console.log(
                                "In Sync: db not completely downloaded"
                            );
                            that.download();
                        });
                    }
        ...
        inc_download: function (options) {
            ...
            this.inc_download_v
                .start_incremental_download(options)
                .done(function () {
                    return dfd.resolve();
                })
                .fail(function (error) {
        ...
        background_download: function () {
            ...
            if (
                this.is_uploadqueue_empty() &&
                this.is_internet_connected() &&
                !this.sync_in_progress
            )
                this.inc_download({
                    background: true,
                })
