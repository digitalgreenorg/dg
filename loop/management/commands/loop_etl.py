...
class LoopStatistics():
    ...
    def recompute_myisam(self):
        ...
        try:
            ...
            df_loopuser = pd.DataFrame(list(LoopUser.objects.filter(role = ROLE_CHOICE_AGGREGATOR).values('id', 'user__id', 'name_en', 'village__block__district__id','village__block__district__state__id', 'village__block__district__state__country__id')))
            ...
            if not result.empty:
                ...
                for index,row in result.iterrows():
                    values_list.append((row['date'].strftime('%Y-%m-%d %H:%M:%S'), str(row['user_created__id']), str(row['mandi__id']), str(row['gaddidar__id']), str(row['quantity__sum']), str(row['amount__sum']), str(row['transportation_cost__sum']), str(row['farmer_share__avg']), str(row['gaddidar_share_amount']), str(row['aggregator_incentive']), row['name'], row['mandi__mandi_name'], row['gaddidar__gaddidar_name'], str(row['distinct_farmer_count']), str(row['country_id']), str(row['state_id']),str(row['district_id'])))

                print "Number of rows to be inserted : ", len(values_list)

                sql_base_query = "INSERT INTO loop_aggregated_myisam (date, aggregator_id, mandi_id, gaddidar_id, quantity, amount, transportation_cost, farmer_share, gaddidar_share, aggregator_incentive, aggregator_name, mandi_name, gaddidar_name, new_distinct_farmer, country_id, state_id, district_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
