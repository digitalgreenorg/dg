...
class Command(BaseCommand):
    ...
    def send_mail(self,total_pending_call_count,
        ...
        email_body = ['<html>',
                    ...
                    '<tr><td>Total Received Calls on %s from %s:00 AM to %s:00 PM</td><td> %s</td></tr>'%(yesterday_date,working_hours_start,working_hours_end,yesterday_received_call_count),
                    '<tr><td>Total Attended Calls on %s from %s:00 AM to %s:00 PM</td><td> %s</td></tr>'%(yesterday_date,working_hours_start,working_hours_end,yesterday_resolved_call_count),
                    ...
                    '<tr><td>Total Received Calls During Off Hours i.e. %s:00 PM, %s to %s:00 AM, %s</td><td> %s</td></tr>'%(working_hours_end,yesterday_date,working_hours_start,today_date,yesterday_off_hours_incoming_call_count),
                    '</table>',
