...
def broadcast(request):
    ...
    if request.method == 'POST':
        if 'broadcast_test_submit' in request.POST:
            ...
            if broadcast_form.is_valid():
                ...
                if (farmer_file):
                    ...
                    village_list = LoopUserAssignedVillage.objects.filter(loop_user_id=cluster_id).values_list(
                        'village', flat=True)
        ...
        Thread(target=start_broadcast,
               args=[broadcast_title, s3_audio_url, farmer_contact_detail, cluster_id, EXOTEL_HELPLINE_NUMBER,
