...
def get_aggregator_local_market(aggregator_market_data):
    ...
    aggregator_local_market = pd.merge(aggregator_market_data, aggregator_local_market, how= 'inner', on= ['Aggregator', 'Total_Quantity'])
...
def find_predicted_cost(daily_farmer_quantity_data, aggregator_local_market, aggregator_market_vehicle_predicted_cost_quantity):
    ...
    daily_farmer_quantity_data_temp = daily_farmer_quantity_data_temp[['Date', 'Aggregator', 'Farmer', 'Quantity_Limit', 'Local_Market', 'Vehicle_ID', 'Predicted_TC', 'Vehicle']]
    ...
    daily_farmer_quantity_data.loc[daily_farmer_quantity_data['Farmer_Quantity'] <= 150, 'Vehicle'] = 'Motor Cycle'
...
def fill_predicted_cost(daily_farmer_quantity_data, daily_transaction_corrected_data):
    ...
    daily_transaction_corrected_data = daily_transaction_corrected_data.drop(['Quantity_Limit', 'Vehicle_ID', 'Vehicle', 'Local_Market_y'], axis= 1)
