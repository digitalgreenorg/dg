...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        animators_to_assign_district = orm['dashboard.Animator'].objects.all()
        for animator in animators_to_assign_district:
            try:
                village = orm['dashboard.Village'].objects.get(id=animator.village.id)
                animator.district_id = village.block.district.id
                animator.save()                            
            except Exception as e:
                print animator.id, e
        ### For unique constraint
        dup_list=orm['dashboard.Animator'].objects.values('name','gender','district', 'partner').annotate(sameper=Count('name')).filter(sameper__gt=1)
        print 'before', len(dup_list)
        for i in dup_list:
            make_unique=orm['dashboard.Animator'].objects.filter(name=i['name'], gender=i['gender'], district=i['district'], partner=i['partner'])
            for j in range(len(make_unique)):
                temp= make_unique[j].name+'('+str(j+1)+')'
                make_unique[j].name=temp
                make_unique[j].save()
                
        dup_list=orm['dashboard.Animator'].objects.values('name','gender','partner').annotate(sameper=Count('name')).filter(sameper__gt=1)
        print 'after', len(dup_list)
