...
def add_person(file, id, block_id):
    ...
    for row in rows:
        if row['village_name'] not in village_map:
            try:
                i = i + 1
                print i
                village = Village(user_created_id = user_id, village_name = row['village_name'], 
                              block_id = block_id)
    ...
    for row in rows2:
        if row['shg_name'] + row['village_name'] not in group_map:
            try:
                i = i + 1
                print i
                group = PersonGroup(user_created_id = user_id, partner_id = user_id.partner.id, group_name = row['shg_name'], 
    ...
    for row in rows2:
        try:
            i = i + 1
            print i
            person = Person(user_created_id = 730, partner_id = user_id.partner.id, person_name = ' '.join([row['Member_Name'], row['Member_Surname']]), 
